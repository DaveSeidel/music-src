  ...
  opcode handleControl, k, iikkk
    iNum, iInch, kMin, kMax, kMonoPolar xin

    kLo1 = -10
    kHi1 = 10

    if (kMonoPolar == 0) then
      kLo2 = -10
    else
      kLo2 = 0
    endif
  ...
  instr 1 
    ...
    koff = handleControl(1, 1, 0.001, 0.1, 1) 
    koff1 = koff 
    koff2 = 2 * koff 
    koff3 = 3 * koff 
    koff4 = 4 * koff 
    ...
    kdst = handleControl(3, 1, 0, 10, 1) 
    aoutL = aL + distort1(aL, kdst, 0.1, 0, 0, 1) 
    aoutR = aR + distort1(aR, kdst, 0.1, 0, 0, 1) 
 
    kpan = handleControl(4, 1, 0, 7, 1) 
